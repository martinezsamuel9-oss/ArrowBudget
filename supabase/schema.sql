...
-- ---------------------------------------------------------------------
-- CATÁLOGO DE INSUMOS
-- IMPORTANTE: se define ANTES que conceptos porque conceptos la referencia
-- ---------------------------------------------------------------------
...
-- Subscriptions: solo lectura para el usuario
...
begin
  ...
  v_pu := v_cd
    + (v_cd * (v_pct_ind / 100))
    + ((v_cd + v_cd * (v_pct_ind / 100)) * (v_pct_imp / 100))
    + ((v_cd + v_cd * (v_pct_ind / 100)) * (v_pct_uti / 100));
